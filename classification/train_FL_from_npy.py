...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_prefix", type=str, default=None,
                        help="If set, save W_g/b_g and normalization prefix_path_*")
    parser.add_argument("--val_ratio", type=float, default=0.2,
                        help="Used only when --val_npy is absent")
    parser.add_argument("--seed", type=int, default=42,
                        help="Random seed for stratified split")
    # --- Bổ sung tham số output file ---
    parser.add_argument("--pred_output", type=str, default="test_predictions.csv",
                        help="Path to save the test prediction csv file")
    ...
    # -------------------------------------------------------------------------
    # PHẦN CODE BỔ SUNG: LƯU PREDICTION RA FILE
    # -------------------------------------------------------------------------
    print(f"Saving test predictions to {args.pred_output}...")
    try:
        # Load dataset gốc (dạng HuggingFace Dataset) để lấy text gốc và các cột metadata khác
        # Lưu ý: split="test" phải khớp với args.test_npy về thứ tự dòng
        ds_test_raw = load_dataset(dataset_name, split="test")
        
        # Chuyển đổi sang Pandas DataFrame để dễ thao tác
        df_test = ds_test_raw.to_pandas()
        
        # Đảm bảo số lượng dòng khớp nhau
        if len(df_test) != len(pred_test):
            print(f"WARNING: Mismatch length! Dataset has {len(df_test)} rows but predictions have {len(pred_test)}. Saving only predictions separately.")
            # Fallback nếu độ dài không khớp (ít khi xảy ra nếu đúng logic)
            df_out = pd.DataFrame({
                "prediction": pred_test.cpu().numpy(),
                "label_id": test_y.cpu().numpy()
            })
        else:
            # Thêm cột dự đoán vào dataframe gốc
            # pred_test đang là tensor trên GPU/CPU, cần chuyển về numpy
            df_test["prediction"] = pred_test.cpu().numpy()
            
            # (Tùy chọn) Nếu bạn muốn chắc chắn cột label ID đúng với model
            # df_test["label_id_verify"] = test_y.cpu().numpy()
            
            df_out = df_test

        # Lưu ra CSV (bỏ index pandas)
        # Tạo thư mục cha nếu chưa tồn tại
        out_dir = os.path.dirname(args.pred_output)
        if out_dir:
            os.makedirs(out_dir, exist_ok=True)
            
        df_out.to_csv(args.pred_output, index=False)
